...
class Net(torch.nn.Module):
    def __init__(self, num_classes=1, in_channels=1):
        ...
        mult_chan = 32
        depth = 4
        intermediate_chan = 25
        self.spec_conv = SubNet2Conv(in_channels,intermediate_chan) #First Spectral Channel Convoultions
        self.spec_down_conv = SubNet2Conv(intermediate_chan, num_classes) #Second spectral Channel Convolutions
        self.spec_down_down_conv = SubNet2Conv(num_classes,1) #Third Spectral Channel Convolution to reduce to 1 channel
        ...
        self.spec_convt = torch.nn.ConvTranspose2d(1, num_classes, kernel_size=3, padding=1) #Transpose convolution to return to desired channel size
        self.spec_bn = torch.nn.BatchNorm2d(num_classes) #batchnorm for transpose conv
        self.spec_relu = torch.nn.ReLU() #ReLu for transpose conv
        self.spec_final = SubNet2Conv(2*num_classes, num_classes) #Final convolution after concatenation of pre and post unet stacks
        self.spec_final_pool = SubNet2Conv(num_classes, 1) #pooling convolution to take you to single channel output
...
if __name__ == '__main__':
    # Test the network
    import tqdm
    net = Net(num_classes=14, in_channels=25 ).cuda()
    x = torch.randn(1, 25, 416, 416).cuda()
    net.eval()
