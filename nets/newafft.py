...
class DEM(nn.Module):
    def __init__(self, dim):
        ...
        self.se1 = SqueezeExcite(dim, rd_ratio=1/16)
        self.se2 = SqueezeExcite(dim, rd_ratio=1/16)
    ...
    def forward(self, inputs1, inputs2):
        ...
        max_pool = self.se1(max_pool)
        avg_pool = self.se2(avg_pool)
        
        dem = self.sigmoid(avg_pool + max_pool)        
        outputs1 = inputs1 * dem
        outputs2 = inputs2 * dem 
...
class CSM(nn.Module):
    def __init__(self, dim):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
        self.conv1_1 = nn.Conv2d(dim, dim // 16, 1, bias=True)
        self.conv1_2 = nn.Conv2d(dim, dim // 16, 1, bias=True)
    ...
    def forward(self, inputs1, inputs2):
        ...
        avg_ = self.avg_pool(add)
        max_ = self.max_pool(add)
        z1 = self.act(self.conv1_1(avg_))
        z2 = self.act(self.conv1_2(max_))
        
        z1 = self.sigmoid(self.conv2_1(z1))
        z2 = self.sigmoid(self.conv2_2(z2))
...
class PatchExpanding(nn.Module):
    def __init__(self, dim, out_dim):
        ...
        self.act = nn.ReLU()
        self.up = nn.ConvTranspose2d(hid_dim, hid_dim, 3, stride=2, padding=1, output_padding=1)
...
class MSCA(nn.Module):    
    ...
    def forward(self, x):
        ...
        x = x + self.norm_1(self.attn(x))
        x = x + self.norm_2(self.ffn(x))
        
        return x 

class NewAFFT(nn.Module):
    ...
    def forward(self, spec, rgb):
        ...
        # final = final.permute(0, 2, 3, 1).softmax(dim=-1)
...
if __name__ == "__main__":
    ...
    model = NewAFFT(num_classes=14, pretrained=False, backbone="resnet18")
